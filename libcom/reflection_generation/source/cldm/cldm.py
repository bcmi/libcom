...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, only_mid_control, reg_net_path, *args, **kwargs):
        ...
        if os.path.exists(reg_net_path):
            state_dict = torch.load(reg_net_path, weights_only=False)['net']
            state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
            self.bbx_reg.load_state_dict(state_dict)
    ...
    def apply_model(self, x_noisy, t, cond, get_stable_features=True, batch=None, *args, **kwargs): # TODO
        ...
        if cond['c_concat'] is None:
            ...
            reflection_features = generate_shadow_clip_features(batch['cls'], batch['objectmask'])

            eps = diffusion_model(noisy_latents=x_noisy, timesteps=t, encoder_hidden_states=cond_txt, down_block_additional_residuals=down_control, mid_block_additional_residual=mid_control, mask_embeddings=reflection_features)
