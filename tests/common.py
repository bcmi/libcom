...
def get_objectstitch_test_list():
    ...
    for img_name in os.listdir(os.path.join(data_dir, 'background')):
        ...
        for k in ['foreground', 'foreground_mask']:
            fg_folder = os.path.join(data_dir, k, img_name.split('.')[0])
            fg_path_list = []
            for per_img_path in os.listdir(fg_folder):
                fg_img_path = os.path.join(data_dir, k, fg_folder, per_img_path)
...
def make_image_grid_mure(img_list, text_list=None, resolution=(512,512), cols=None, border_color=255, border_width=5):
    if cols == None:
        cols = len(img_list)
    assert len(img_list) % cols == 0, f'{len(img_list)} % {cols} != 0'
    if isinstance(text_list, (list, tuple)):
